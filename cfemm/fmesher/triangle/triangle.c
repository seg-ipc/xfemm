...
/* uintptr_t is always pointer-sized on all platforms.
   On Windows x64, sizeof(TRIANGLE_ULONG)==4 but sizeof(void*)==8.
   Triangle historically used unsigned long for pointer arithmetic,
   which silently truncates pointers on Windows x64 and causes crashes. */
#include <stdint.h>
#define TRIANGLE_ULONG uintptr_t
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((TRIANGLE_ULONG) (ptr) & (TRIANGLE_ULONG) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((TRIANGLE_ULONG) (ptr) ^ (TRIANGLE_ULONG) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((TRIANGLE_ULONG) (otri).tri | (TRIANGLE_ULONG) (otri).orient)
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
                    ((TRIANGLE_ULONG) (otri).tri[6] | (TRIANGLE_ULONG) 2l)
...
#define infected(otri)                                                        \
  (((TRIANGLE_ULONG) (otri).tri[6] & (TRIANGLE_ULONG) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((TRIANGLE_ULONG) (sptr) & (TRIANGLE_ULONG) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((TRIANGLE_ULONG) (sptr) & ~ (TRIANGLE_ULONG) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((TRIANGLE_ULONG) (osub).ss | (TRIANGLE_ULONG) (osub).ssorient)
...

{
  ...
  if (printvertex == (vertex) NULL)
    ...
    TriMessage("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (TRIANGLE_ULONG) printvertex,
  ...
  if (printvertex == (vertex) NULL)
    ...
    TriMessage("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (TRIANGLE_ULONG) printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    ...
    TriMessage("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
           t->orient + 3, (TRIANGLE_ULONG) printvertex,
           printvertex[0], printvertex[1]);

  if (b->usesegments) {
    sdecode(t->tri[6], printsh);
    if (printsh.ss != m->dummysub) {
      TriMessage("    [6] = x%lx  %d\n", (TRIANGLE_ULONG) printsh.ss,
             printsh.ssorient);
    }
...

{
  ...
  TriMessage("subsegment x%lx with orientation %d and mark %d:\n",
         (TRIANGLE_ULONG) s->ss, s->ssorient, mark(*s));
  ...
  if (printvertex == (vertex) NULL)
    ...
    TriMessage("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           2 + s->ssorient, (TRIANGLE_ULONG) printvertex,
           printvertex[0], printvertex[1]);
  sdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    TriMessage("    Dest  [%d] = NULL\n", 3 - s->ssorient);
    ...
    TriMessage("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           3 - s->ssorient, (TRIANGLE_ULONG) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    TriMessage("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
           4 + s->ssorient, (TRIANGLE_ULONG) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    TriMessage("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
           5 - s->ssorient, (TRIANGLE_ULONG) printvertex,
           printvertex[0], printvertex[1]);
}
...

{
  ...
  alignptr = (TRIANGLE_ULONG) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
    (alignptr + (TRIANGLE_ULONG) pool->alignbytes -
     (alignptr % (TRIANGLE_ULONG) pool->alignbytes));
...

{
  ...
  alignptr = (TRIANGLE_ULONG) (pool->pathblock + 1);
...

{
  ...
  alignptr = (TRIANGLE_ULONG) m->dummytribase;
  m->dummytri = (triangle *)
    (alignptr + (TRIANGLE_ULONG) m->triangles.alignbytes -
     (alignptr % (TRIANGLE_ULONG) m->triangles.alignbytes));
  ...
  if (b->usesegments) {
    ...
    alignptr = (TRIANGLE_ULONG) m->dummysubbase;
    m->dummysub = (subseg *)
      (alignptr + (TRIANGLE_ULONG) m->subsegs.alignbytes -
       (alignptr % (TRIANGLE_ULONG) m->subsegs.alignbytes));
...

{
  ...
  alignptr = (TRIANGLE_ULONG) (getblock + 1);
  foundvertex = (char *) (alignptr + (TRIANGLE_ULONG) m->vertices.alignbytes -
                          (alignptr % (TRIANGLE_ULONG) m->vertices.alignbytes));
...

{
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (TRIANGLE_ULONG) (sampleblock + 1);
    firsttri = (char *) (alignptr +
                         (TRIANGLE_ULONG) m->triangles.alignbytes -
                         (alignptr %
                          (TRIANGLE_ULONG) m->triangles.alignbytes));
